...
async function installSpec(spec) {
  ...
  // Quote the spec for the shell. cmd.exe recognizes double quotes (not
  // single), so Windows uses double quotes; the guard above already refused
  // every metacharacter including `"`, so double-quoting cannot be escaped.
  // POSIX uses double quotes too (single quotes are safe for `'`-free specs,
  // but the guard refuses `'` anyway). `-w` is required because the profile
  // carries a pnpm-workspace.yaml, so pnpm treats `add` as workspace-root.
  const command = `dsh plugin --profile web add -w "${clean}"`
    ...
    const installHandler = async (req, res) => {
      try {
        ...
        try {
          ...
          spec = String((body && body.spec) || '').trim()
        } catch {
          // tolerate non-JSON or empty body
        }
        ...
        res.writeHead(500, { 'content-type': 'application/json; charset=utf-8' })
        res.end(JSON.stringify({ ok: false, installed: '', output: '', requiresRestart: false, error: String((e && e.message) || e) }))
      }
    }
