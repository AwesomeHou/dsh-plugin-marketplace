...
async function buildInstalledList() {
  ...
  // Manageable (installed) first, updates first within them, then builtins.
  plugins.sort((a, b) => {
    if (a.kind !== b.kind) return a.kind === 'installed' ? -1 : 1
    if (a.kind === 'installed') return (b.updateAvailable ? 1 : 0) - (a.updateAvailable ? 1 : 0)
    return String(a.name).localeCompare(String(b.name))
  })
  return { plugins, self, error: null }
}
