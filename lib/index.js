...
// ── pnpm store-path pinning ────────────────────────────────────────────────
// pnpm v10+ records the store path it used to build `node_modules` in
// `<profile>/node_modules/.modules.yaml` (`storeDir`, e.g. `…\v10`) and HARD-
// FAILS with ERR_PNPM_UNEXPECTED_STORE (or, in a no-TTY context,
// ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY) whenever a mutating install runs
// with a different effective store. The default store path is computed from
// process-context env vars (USERPROFILE/LOCALAPPDATA), which DSH Desktop's
// Electron process can resolve differently between launches (e.g. to
// `C:\Users` instead of `C:\Users\<user>`), so the store "drifts" and every
// GUI install dies with exit 1.
//
// Pin the store with `--config.store-dir=<dir>` on every `dsh plugin`
// invocation. That CLI flag is the one mechanism every pnpm version honors:
//   - `npm_config_store_dir` env  → pnpm ≤10 honors, pnpm ≥11 ignores.
//   - project `.npmrc` store-dir   → pnpm ≤10 honors, pnpm ≥11 ignores
//     (security: like npm, pnpm refuses to let a project redirect the cache).
//   - `--config.store-dir` flag    → honored by pnpm 9/10/11 (verified).
// The pinned dir adopts the store already recorded in `.modules.yaml` (so the
// existing cache is reused and pnpm's current store always matches the
// recorded one), else a stable per-profile default for fresh profiles.

/** Recorded store path in the profile's node_modules/.modules.yaml, or null. */
function recordedModulesStoreDir() {
  const file = join(webProfileDir(), 'node_modules', '.modules.yaml')
  if (!existsSync(file)) return null
  try {
    const txt = readFileSync(file, 'utf8')
    // pnpm writes `  "storeDir": "C:\\…\\v10",` (JSON-quoted) or `storeDir: <path>`.
    let m = /["']storeDir["']\s*:\s*["']((?:[^"'\\]|\\.)*)["']/.exec(txt)
    let raw = m ? m[1] : null
    if (!raw) {
      m = /^storeDir:\s*(.+?)\s*$/m.exec(txt)
      raw = m ? m[1] : null
    }
    if (!raw) return null
    // Unescape a JSON-style quoted value (C:\\ → C:\).
    if (raw.includes('\\')) { try { raw = JSON.parse('"' + raw + '"') } catch { /* keep raw */ } }
    const v = raw.trim()
    return v ? v : null
  } catch { return null }
}
...
/**
 * The pnpm store BASE dir to pin for the web profile. `.modules.yaml` records
 * the VERSIONED store dir (`…\v10`); pnpm's `store-dir` config is the base and
 * pnpm re-appends `\v<major>`, so the trailing `\v<digits>` is stripped before
 * adoption.
 */
function effectiveStoreDir() {
  const recorded = recordedModulesStoreDir()
  if (recorded) {
    const base = recorded.replace(/[\\/]v\d+$/, '')
    if (base) return base
  }
  return join(webProfileDir(), '.pnpm-store')
}

/**
 * The `--config.store-dir=<dir>` token to append to a `dsh plugin` invocation,
 * or null when the path contains shell metacharacters (never inject then).
 * @returns {string|null}
 */
function storeDirArg() {
  const dir = effectiveStoreDir()
  return /[&|<>^()%"'!]/.test(dir) ? null : '--config.store-dir=' + dir
}
...
function runDshAsync(args, { env, onLine } = {}) {
  // Pin the pnpm store on every profile-pnpm op via `--config.store-dir` so it
  // can never drift between process contexts (see the store-pinning comment).
  // The flag is the one mechanism every pnpm version honors; skip it only if
  // the store path itself would break the shell command line.
  const storeArg = storeDirArg()
  const dshTokens = resolveDshCommand()
  return runProc(quote([...dshTokens, 'plugin', '--profile', 'web', ...(storeArg ? [...args, storeArg] : args)]), { env, onLine })
}
...
function classifyPnpmFailure(output) {
  ...
  // The store path recorded in node_modules/.modules.yaml no longer matches the
  // current effective store (USERPROFILE/LOCALAPPDATA drift between DSH Desktop
  // launches, or a different pnpm major). It is a hard error until reconciled.
  // The marketplace pins store-dir via `--config.store-dir` (see the store
  // pinning helpers), so this normally only surfaces before the pin applies;
  // when it does, rebuild + retry with the pinned store resolves it.
  if (/ERR_PNPM_UNEXPECTED_STORE|ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY|Unexpected store location|are currently linked from the store at/.test(out)) {
    return { code: 'unexpected-store', recoverable: true,
      message: 'pnpm 缓存 store 路径与 node_modules 记录的不一致（多为 DSH Desktop 重启后进程上下文里的 USERPROFILE/LOCALAPPDATA 变化所致）。市场已自动把 store-dir 固定到记录路径并重建重试；若仍失败请重启 harness 后再试。' }
  }
...
export const _market = {
  ...
  needsRestart,
  buildInstalledList,
  // test surface for the pnpm store-drift fix
  recordedModulesStoreDir,
  effectiveStoreDir,
  storeDirArg,
}
