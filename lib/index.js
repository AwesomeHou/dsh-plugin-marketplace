...
async function runDsh(args) {
  ...
  if (!ok && /(not recognized as an internal|command not found|不是内部或外部命令)/i.test(output)) {
    return { ok: false, output, error: `找不到 dsh 可执行文件（已尝试解析为 ${dshTokens.join(' ')}）。请把 dsh 加入 PATH，或通过 dsh CLI 启动 harness 后重试。` }
  }
