...
function runDshAsync(args, { env, onLine } = {}) {
  ...
  const command = [...dshTokens, 'plugin', '--profile', 'web', ...args].map((a) => `"${a}"`).join(' ')
  let child = null
...
function finishJob(job, r) {
  ...
  job.resultResolve(job.kind === 'update'
    ? { ...base, name: job.name || job.spec, installed: job.spec }
    : { ...base, installed: job.spec })
}
...
async function executeJob(job) {
  try {
    ...
    const run = runDshAsync(args, { env, onLine: (line) => handleLine(job, line) })
    job.childKill = run.kill
    let r = await run.promise
    if (!r.ok && job.proxy && !job.canceled && !job.timedOut) {
      // The loopback proxy may be the culprit on machines that need a special
      // proxy to reach the internet; retry once direct so it can never block
      // a real install.
      job.log.push('(本地字节统计代理不可用，改为直连重试…)')
      job.output += '(本地字节统计代理不可用，改为直连重试…)\n'
      if (job.proxy) { try { job.proxy.close() } catch { /* noop */ } job.proxy = null }
      const run2 = runDshAsync(args, { env: { ...process.env }, onLine: (line) => handleLine(job, line) })
      job.childKill = run2.kill
      r = await run2.promise
    }
...
function startUpdate(name) {
  ...
  return { job: startJob('update', target, name) }
}
