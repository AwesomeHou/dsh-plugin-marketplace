...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Send a prompt to the current agent session (the same RPC the chat
     * composer uses). This hands the task to the agent instead of running the
     * marketplace's own (workspace-dependent) install machinery.
     * @returns Promise resolving once the prompt is accepted (the agent then
     *   works on it in the conversation).
     */
    function sendToAgent(text) {
      var rpc = function rpc(method, payload) {
      ...
      return rpc('session.list', {})
        .then(function (res) {
          ...
          return rpc('session.prompt', { sessionId: session.sessionId, mode: 'queue', content: [{ type: 'text', text: text }] })
        })
        .then(function (res) {
          ...
          : (current.items.length === 0
            ...
            : React.createElement('div', null,
              current.items.map(function (it) {
                var key = it.fullName
                var st = installs[key] || { status: 'idle' }
                ...
                return React.createElement('div', { key: it.url + it.fullName, className: 'dsh-market-card' },
                  ...
                  sending
                    ...
                    : (sent
                      ? React.createElement('div', { className: 'dsh-market-install-note' }, '已交给 agent 处理。请到对话中查看它的分析与安装结果。')
                      : (installing
