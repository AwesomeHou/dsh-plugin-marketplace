...
- **Agent 工具**（Host 侧通过 `ctx.tools.register` 注册）：
  ...
  - `market_installed()` — 列出 `web` profile 已安装的插件：内置 / 后安装、启用状态、当前版本、最新版本与是否可更新（含市场自身的更新状态）。
...
- **区分内置 / 后安装** — `dsh.profile.bundles` 里来自 profile 模板的包是**内置**插件（随 harness 提供，不能关闭 / 卸载）；后加入 `dependencies` 的是**后安装**插件。
